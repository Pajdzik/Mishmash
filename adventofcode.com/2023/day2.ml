...
let process_part1 input_lines =
  let parse_line line =
    let parse_id str =
      ...
      match parts with [ _; id ] -> int_of_string id | _ -> failwith "No id"
    in
    ...
    let parse_games str =
      let parse_game game_str =
        let check_for_color str =
          ...
          | game -> failwith ("Unparsable colors: " ^ "'" ^ str ^ "'")
        in

        (* "3 blue, 4 red" *)
        let parts = String.split_on_char ',' game_str in
        List.for_all check_for_color parts
      in

      (* "3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green" *)
      let game_parts = String.split_on_char ';' str in
      List.for_all parse_game game_parts
    in
  ...
  let process_line line =
    ...
    | line -> failwith ("Unexpected line: " ^ line)
  in

  let results = List.map process_line input_lines in
  let positive_results =
    List.filter (fun (id, result) -> result = true) results
  in
...
let process_part2 input_lines =
  let parse_color color_str =
    ...
    | _ -> failwith ("Unknown color format: " ^ color_str)
  in
...
let () = read_lines "day2.txt" |> process_part2 |> print_int
